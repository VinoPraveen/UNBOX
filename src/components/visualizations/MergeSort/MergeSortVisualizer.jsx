...
const POINTER_KIND_CLASS = {
  left: 'mrsv__pointer--left',
  right: 'mrsv__pointer--right',
  write: 'mrsv__pointer--write',
  range: 'mrsv__pointer--range',
};
...
function rangeCopy(snapshot, n) {
  const { phase, region } = snapshot;
  if (!region) return `Array of ${n} elements`;
  if (phase === 'divide' && region.mid >= 0) {
    return `Split [${region.lo}\u2013${region.mid}] + [${region.mid + 1}\u2013${region.hi}]`;
  }
  if (phase === 'merge' && region.mid >= 0) {
    return `Merge [${region.lo}\u2013${region.mid}] + [${region.mid + 1}\u2013${region.hi}]`;
  }
  return `Range [${region.lo}\u2013${region.hi}]`;
}

export default function MergeSortVisualizer({ complexity, snapshot }) {
  if (!snapshot) {
    return (
  ...
  const cutLabel = snapshot.phase === 'divide' ? 'SPLIT' : snapshot.phase === 'merge' ? 'MERGE' : null;

  return (
    <div className="aviz">
      <div className="aviz__stage">
        <div className="mrsv__context" aria-label="Merge sort context">
          <div className="mrsv__pipeline" role="group" aria-label="Merge sort phase">
            {MERGE_PHASES.map(([key, label]) => (
              <span
                key={key}
                className={'mrsv__phase' + (snapshot.phase === key ? ' mrsv__phase--active' : '')}
              >
                {label}
              </span>
            ))}
          </div>
          <span className="mrsv__range">{rangeCopy(snapshot, n)}</span>
        </div>
        ...
        <div className={'aviz__scroll' + (huge ? ' aviz__scroll--huge' : '')}>
          ...
          {cutLabel && (
            <div className="mrsv__tags" aria-hidden="true">
              {dividers.map((index) => (
                <span className="mrsv__tag" key={`cut-${index}`}>
                  {cutLabel}
                </span>
              ))}
            </div>
